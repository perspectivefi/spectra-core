 ...
 * maxDeposit decreased by 1, getTestUpgradeability, setTestUpgradeability methods and using MockPrincipalTokenUtilV2.
 */
    ...
    IPrincipalToken
{
    ...
    using MockPrincipalTokenUtilV2 for address;
    using MockPrincipalTokenUtilV2 for uint256;
    ...
    address private testUpgradeability;
    ...
    function initialize(
        ...
        string memory _name = string.concat(NamingUtil.genPTName(_ibtSymbol, expiry), "V2");
        __ERC20_init(_name, string.concat(NamingUtil.genPTSymbol(_ibtSymbol, expiry), "V2"));
    ...
    function setTestUpgradeability(address _testUpgradeability) public {
    ...
    function maxDeposit(address /* receiver */) external view override returns (uint256) {
        return paused() || (block.timestamp >= expiry) ? 0 : type(uint256).max - 1;
    }
    ...
    /** @dev Used for upgradeability testing */
    function getTestUpgradeability() public view virtual returns (address) {
