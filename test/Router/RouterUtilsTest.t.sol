...
contract ContractRouterUtilTest is RouterBaseTest {
    ...
    function testRouterUtilSetter() public {
        bytes memory revertData = abi.encodeWithSelector(
            bytes4(keccak256("AccessManagedUnauthorized(address)")),
            other
        );
        assertEq(router.routerUtil(), address(routerUtil));

        vm.prank(other);
        vm.expectRevert(revertData);
        router.setRouterUtil(address(9));

        vm.prank(scriptAdmin);
        accessManager.grantRole(Roles.UPGRADE_ROLE, other, 0);
        vm.prank(other);
        router.setRouterUtil(address(9));

        assertEq(router.routerUtil(), address(9));
    }
    ...
    function testSpotExchangeRateValidity() public {
        ...
        assertTrue(ibtInPT >= Constants.UNIT);

        uint256 ptInIBT = routerUtil.spotExchangeRate(address(curvePool), 1, 0);
        assertTrue(ptInIBT <= Constants.UNIT);
    }
    ...
    function testExactYTToIBTFlashSwapFuzz(uint256 inputYTAmount) public {
        ...
        uint256 priceImpactAndFees = _getCurvePriceImpactLossAndFees(0, 1, borrowedIBTAmount)
            .mulDiv(routerUtil.spotExchangeRate(address(curvePool), 1, 0), Constants.UNIT);
    ...
    function testSecantMethodPrecisionFuzz(uint256 inputIBTAmount) public {
        ...
        (uint256 maxGuess, , ) = routerUtil.previewFlashSwapExactIBTToYT(
            address(curvePool),
            inputIBTAmount
        );

        uint256 ytWanted = maxGuess;
        ...
        assertApproxEqRel(inputIBTAmount, inputIBTAmountRequired, 1000 * ERROR_MARGIN);
    }
    ...
    function testFlashSwapCycleFuzz(uint256 inputIBTAmount) public {
        ...
        // We swap IBT for PT. We are left with maxGuess PT, that we swap back for IBT. We evalute the loss
        // associated to this swap
        uint256 priceImpactAndFees1 = _getCurvePriceImpactLossAndFees(1, 0, maxGuess);

        (uint256 outputIBTAmount, uint256 borrowedIBTAmount2) = routerUtil
            .previewFlashSwapExactYTToIBT(address(curvePool), maxGuess);
    ...
    function _getCurvePriceImpactLossAndFees(
        ...
        return
            amountIn.mulDiv(routerUtil.spotExchangeRate(address(curvePool), i, j), Constants.UNIT) -
