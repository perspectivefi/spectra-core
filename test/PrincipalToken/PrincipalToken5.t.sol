...
contract ContractPrincipalToken5 is Test {
    /* STRUCTS */
    struct DepositData {
        // before
    ...
    function setUp() public {
        ...
        vm.prank(scriptAdmin);
        accessManager.grantRole(Roles.REWARDS_HARVESTER_ROLE, scriptAdmin, 0);
        vm.prank(scriptAdmin);
        accessManager.grantRole(Roles.REWARDS_PROXY_SETTER_ROLE, scriptAdmin, 0);
    }

    /* UNIT TESTS */

    function testSetupRewardsProxyContract() public {
        rewardsProxy = new MockRewardsProxy();
        vm.expectEmit(true, true, false, true);
        emit RewardsProxyChange(address(0), address(rewardsProxy));
        bytes memory revertData = abi.encodeWithSelector(
            bytes4(keccak256("AccessManagedUnauthorized(address)")),
            address(this)
        );
        vm.expectRevert(revertData);
        principalToken.setRewardsProxy(address(rewardsProxy));
        vm.prank(scriptAdmin);
        principalToken.setRewardsProxy(address(rewardsProxy));
    }
    ...
    function testEnableClaimAndDisableRewardsProxy() public {
        // We set a rewards proxy
        rewardsProxy = new MockRewardsProxy();
        vm.expectEmit(true, true, false, true);
        emit RewardsProxyChange(address(0), address(rewardsProxy));
        vm.prank(scriptAdmin);
        ...
        // user address(1) deposits 500e18 underlying in the PT and 500e18 underlying in the IBT
        underlying.mint(address(1), 1000e18);
        vm.startPrank(address(1));
        underlying.approve(address(principalToken), 500e18);
        principalToken.deposit(500e18, address(1));
        underlying.approve(address(ibt), 500e18);
        ibt.deposit(500e18, address(1));
        vm.stopPrank();

        // We can use it and claim rewards
        incentivesContract.testAddClaimable(100e18);
        uint256 claimableByPT = incentivesContract.claimableByUser(address(principalToken));
        ...
        vm.expectEmit(true, true, false, true);
        emit Claimed(address(principalToken), claimableByPT);
        vm.prank(scriptAdmin);
        principalToken.claimRewards(claimData); // The PT did claim the rewards
        assertEq(rewardToken.balanceOf(feeCollector), claimableByPT);

        // We disable it
        vm.expectEmit(true, true, false, true);
        emit RewardsProxyChange(address(rewardsProxy), address(0));
        vm.prank(scriptAdmin);
    ...
    function testClaimRewardsWithoutAccessControl() public {
        // We set a rewards proxy
        rewardsProxy = new MockRewardsProxy();
        vm.expectEmit(true, true, false, true);
        emit RewardsProxyChange(address(0), address(rewardsProxy));
