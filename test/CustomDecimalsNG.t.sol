...
// @dev Tests involving pools use the NG implementation
contract CustomDecimalsNG is Test {
    ...
    function testCDCurveLiqArbitrageFuzz(
        uint8 _underlyingDecimals,
        uint8 _ibtDecimals,
        uint256 _amount,
        uint256 _swapAmount
        ...
        data.baseProportion1 = CurveLiqArbitrage(curveLiqArbitrage).findBestProportionNG(
            curvePool,
            depositAmount,
            1e5
        );
        ...
        uint256 amount1 = IBT_UNIT.mulDiv(
            UNIT,
            data.baseProportion1 + ICurveNGPool(curvePool).last_prices()
        );
        uint256 amount0 = amount1.mulDiv(data.baseProportion1, UNIT);
        data.lpAmount1 = ICurveNGPool(curvePool).calc_token_amount([amount0, amount1], true);
        uint256 swapAmountPreviewed = bound(_amount, 10 ** (ibtDecimals / 2 + 1), amount0);
        data.lpAmount2 = ICurveNGPool(curvePool).calc_token_amount(
            [
                amount0 - swapAmountPreviewed,
                amount1 + ICurveNGPool(curvePool).get_dy(0, 1, swapAmountPreviewed)
            ],
            true
        );
        swapAmountPreviewed = bound(_amount, 10 ** (ibtDecimals / 2), amount1);
        data.lpAmount3 = ICurveNGPool(curvePool).calc_token_amount(
            [
                amount0 + ICurveNGPool(curvePool).get_dy(1, 0, swapAmountPreviewed),
        ...
        uint256 swapAmount = bound(
            ...
            ICurveNGPool(curvePool).balances(1).mulDiv(ASSET_UNIT, 2 * IBT_UNIT)
        );
        ...
        ICurveNGPool(curvePool).exchange(0, 1, data.ibtReceived, 0, testUser);

        data.baseProportion2 = CurveLiqArbitrage(curveLiqArbitrage).findBestProportionNG(
            curvePool,
            depositAmount,
            1e3
        );
        amount1 = IBT_UNIT.mulDiv(
            UNIT,
            data.baseProportion2 + ICurveNGPool(curvePool).last_prices()
        );
        ...
        swapAmount = bound(
            ...
            ICurveNGPool(curvePool).balances(0).mulDiv(ASSET_UNIT, 2 * IBT_UNIT)
        );
        MockUnderlyingCustomDecimals(underlying).mint(testUser, swapAmount);
        ...
        ICurveNGPool(curvePool).exchange(1, 0, data.ptReceived, 0, testUser);

        data.baseProportion3 = CurveLiqArbitrage(curveLiqArbitrage).findBestProportionNG(
            curvePool,
            depositAmount,
            1e2
        );
        amount1 = IBT_UNIT.mulDiv(
            UNIT,
            data.baseProportion3 + ICurveNGPool(curvePool).last_prices()
        );
        amount0 = amount1.mulDiv(data.baseProportion3, UNIT);
        data.lpAmount1 = ICurveNGPool(curvePool).calc_token_amount([amount0, amount1], true);
        swapAmountPreviewed = bound(_swapAmount, 10 ** (ibtDecimals / 2 + 1), amount0);
        data.lpAmount2 = ICurveNGPool(curvePool).calc_token_amount(
            [
                amount0 - swapAmountPreviewed,
                amount1 + ICurveNGPool(curvePool).get_dy(0, 1, swapAmountPreviewed)
        ...
        swapAmountPreviewed = bound(_swapAmount, 10 ** (ibtDecimals / 2 + 1), amount1);
        data.lpAmount3 = ICurveNGPool(curvePool).calc_token_amount(
            [
                amount0 + ICurveNGPool(curvePool).get_dy(1, 0, swapAmountPreviewed),
                amount1 - swapAmountPreviewed
        ...
        assertGe(
            data.lpAmount1,
            data.lpAmount2.mulDiv(999, 1000),
            "Best proportion found 3 is sub optimal 1"
        );
        assertGe(
            data.lpAmount1,
            data.lpAmount3.mulDiv(999, 1000),
    ...
    function testCDPreviewUnitaryAddLiquidityFuzz(
        ...
        ibtAmount = bound(ibtAmount, IBT_UNIT, 1_000_000_000_000 * IBT_UNIT);
        data.baseProportion1 = ICurveNGPool(curvePool).balances(0).mulDiv(
            UNIT,
        ...
        data.previewLPTReceived1 = CurveLiqArbitrage(curveLiqArbitrage)
            .previewNGUnitaryAddLiquidity(curvePool, ibtAmount, data.baseProportion1);
        data.previewLPTReceived12 = CurveLiqArbitrage(curveLiqArbitrage)
            .previewNGUnitaryAddLiquidity(curvePool, ibtAmount / 10, data.baseProportion1);
        data.previewLPTReceived13 = CurveLiqArbitrage(curveLiqArbitrage)
            .previewNGUnitaryAddLiquidity(curvePool, ibtAmount * 10, data.baseProportion1);
        assertApproxEqRel(
        ...
        data.baseProportion2 = ICurveNGPool(curvePool).balances(0).mulDiv(
            UNIT,
            ICurveNGPool(curvePool).balances(1)
        );
        assertGt(
        ...
        // adding in different amount should not result in different result
        data.previewLPTReceived1 = CurveLiqArbitrage(curveLiqArbitrage)
            .previewNGUnitaryAddLiquidity(curvePool, ibtAmount, data.baseProportion2);
        data.previewLPTReceived12 = CurveLiqArbitrage(curveLiqArbitrage)
            .previewNGUnitaryAddLiquidity(curvePool, ibtAmount / 10, data.baseProportion2);
        data.previewLPTReceived13 = CurveLiqArbitrage(curveLiqArbitrage)
            .previewNGUnitaryAddLiquidity(curvePool, ibtAmount * 10, data.baseProportion2);
        assertApproxEqRel(
            data.previewLPTReceived1,
        ...
        ICurveNGPool(curvePool).exchange(1, 0, data.ptReceived, 0, testUser);
        // proportion should now be greater than before
        data.baseProportion3 = ICurveNGPool(curvePool).balances(0).mulDiv(
            UNIT,
            ICurveNGPool(curvePool).balances(1)
        );
        assertGt(
        ...
        // adding in different amount should not result in different result
        data.previewLPTReceived1 = CurveLiqArbitrage(curveLiqArbitrage)
            .previewNGUnitaryAddLiquidity(curvePool, ibtAmount, data.baseProportion3);
        data.previewLPTReceived12 = CurveLiqArbitrage(curveLiqArbitrage)
            .previewNGUnitaryAddLiquidity(curvePool, ibtAmount / 10, data.baseProportion3);
        data.previewLPTReceived13 = CurveLiqArbitrage(curveLiqArbitrage)
            .previewNGUnitaryAddLiquidity(curvePool, ibtAmount * 10, data.baseProportion3);
        assertApproxEqRel(
            data.previewLPTReceived1,
    ...
    function testCDFindBestProportionOptimalFuzz(
        ...
        uint256 bestProp = CurveLiqArbitrage(curveLiqArbitrage).findBestProportionNG(
            curvePool,
            ibtAmount,
            epsilon
        );
        ...
        uint256 tradePropRate = RouterUtil(routerUtil).previewNGAddLiquidityWithIBT(
            curvePool,
            IBT_UNIT
        );

        uint256 bestPropRate = CurveLiqArbitrage(curveLiqArbitrage).previewNGUnitaryAddLiquidity(
            curvePool,
            ibtAmount,
            bestProp
        );
        ...
        uint256 expectedPropRate = CurveLiqArbitrage(curveLiqArbitrage)
            .previewNGUnitaryAddLiquidity(curvePool, ibtAmount, 0.8e18);
        assertGe(
            bestPropRate,
            tradePropRate,
        ...
        uint256 randomPropRate = CurveLiqArbitrage(curveLiqArbitrage).previewNGUnitaryAddLiquidity(
            curvePool,
            ibtAmount,
            randomProp
        );
    ...
    function testCDFindBestProportionFuzz(
        ...
        swapAmount = bound(
            swapAmount,
            ICurveNGPool(curvePool).balances(1) / (10 ** (data.ibtDecimals / 2)),
            ICurveNGPool(curvePool).balances(1) / 2
        );
        ...
        swapAmount = bound(
            ibtAmount,
            ICurveNGPool(curvePool).balances(0) / (10 ** (data.ibtDecimals / 2)),
            ICurveNGPool(curvePool).balances(0) / 2
        );
        swapAmount = swapAmount.mulDiv(ASSET_UNIT, IBT_UNIT);
        MockUnderlyingCustomDecimals(underlying).mint(testUser, swapAmount);
        IERC20(underlying).approve(address(IPrincipalToken(pt)), swapAmount);
        data.ptReceived = IPrincipalToken(pt).deposit(swapAmount, testUser);
        IPrincipalToken(pt).approve(curvePool, data.ptReceived);
        if (data.ptReceived < ICurveNGPool(curvePool).balances(1) / 10) {
            ICurveNGPool(curvePool).exchange(1, 0, data.ptReceived, 0, testUser);
        } else {
            ICurveNGPool(curvePool).exchange(
                1,
                0,
                ICurveNGPool(curvePool).balances(1) / 10,
                0,
                testUser
            );
        }
    ...
    function testSwapUnderlyingToPTFuzz(
        ...
        uint256 dy = ICurveNGPool(curvePool).get_dy(
            0,
            1,
            IERC4626(ibt).convertToShares(swapAmount)
        );
    ...
    function testFlashSwapIBTToExactYTFuzz(
        ...
        outputYTAmount = bound(outputYTAmount, IBT_UNIT, ICurveNGPool(curvePool).balances(1) / 10);

        // * Pre-compute input values
        (uint256 inputIBTAmount, uint256 borrowedIBTAmount) = RouterUtil(routerUtil)
            .previewNGFlashSwapIBTToExactYT(curvePool, outputYTAmount);
    ...
    function testFlashSwapExactIBTToYTFuzz(
        ...
        inputIBTAmount = bound(inputIBTAmount, IBT_UNIT, ICurveNGPool(curvePool).balances(0) / 100);

        // * Pre-compute input values
        (uint256 outputYTAmount, uint256 borrowedIBTAmount) = RouterUtil(routerUtil)
            .previewNGFlashSwapExactIBTToYT(curvePool, inputIBTAmount);
        ...
        IRouter(router).execute(commands, inputs);
        assertApproxEqRel(ibtBalBefore - IERC20(ibt).balanceOf(testUser), inputIBTAmount, 1e15);
        assertEq(IERC20(yt).balanceOf(testUser), outputYTAmount);
    }

    function testFlashSwapExactYTToIBTFuzzCDNG(
        ...
        inputYTAmount = bound(inputYTAmount, IBT_UNIT, ICurveNGPool(curvePool).balances(1) / 10);

        // * Pre-compute input values
        (uint256 outputIBTAmount, uint256 borrowedIBTAmount) = RouterUtil(routerUtil)
            .previewNGFlashSwapExactYTToIBT(curvePool, inputYTAmount);
    ...
    function testSecantMethodPrecisionCustomDecimalsFuzz(
        ...
        inputIBTAmount = bound(inputIBTAmount, IBT_UNIT, ICurveNGPool(curvePool).balances(1) / 100);
        (uint256 outputYTAmount, ) = RouterUtil(routerUtil).previewNGFlashSwapExactIBTToYT(
            curvePool,
            inputIBTAmount
        );

        (uint256 inputIBTAmountRequired, ) = RouterUtil(routerUtil).previewNGFlashSwapIBTToExactYT(
            curvePool,
            outputYTAmount
        );
    ...
    function testIBTToExactYTFlashSwapCustomDecimalsFuzz(
        ...
        ytWanted = bound(ytWanted, IBT_UNIT, ICurveNGPool(curvePool).balances(1) / 100);
        ...
        (uint256 amountIBTNeeded, ) = RouterUtil(routerUtil).previewNGFlashSwapIBTToExactYT(
            curvePool,
            ytWanted
        );
    ...
    function testExactYTToIBTFlashSwapCustomDecimalsFuzz(
        ...
        inputYTAmount = bound(inputYTAmount, IBT_UNIT, ICurveNGPool(curvePool).balances(0) / 100);
        uint256 ytIBTSpot = RouterUtil(routerUtil).convertIBTToYTSpot(IBT_UNIT, curvePool);
    ...
    function testFlashSwapCycleCustomDecimalsFuzz(
        ...
        (uint256 outputIBTAmount, uint256 borrowedIBTAmount2) = RouterUtil(routerUtil)
            .previewNGFlashSwapExactYTToIBT(curvePool, outputYTAmount);
    ...
    function _testRouterSwap(
        ...
        data.expected2 = ICurveNGPool(curvePool).get_dy(i, j, amountIn);
    ...
    function _getCurvePriceImpactLossAndFees(
        ...
        return
            amountIn.mulDiv(
                ...
                CurvePoolUtil.CURVE_UNIT
            ) - ICurveNGPool(curvePool).get_dy(i, j, amountIn);
    }
}
