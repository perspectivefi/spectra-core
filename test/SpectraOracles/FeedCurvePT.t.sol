...
import {MockPriceFeedCurvePTAsset} from "./mocks/cryptoswap-ng/MockPriceFeedCurvePTAsset.sol";
...
contract FeedCurvePTTest is BaseSpectraFeedTest {
    ...
    function test_description() public override {
        ...
        _priceFeedInAsset_ = address(new MockPriceFeedCurvePTAsset(_pt_, _curvePool_));
        _priceFeedInIBT_ = address(new MockPriceFeedCurvePTIBT(_pt_, _curvePool_));
        assertEq(
    ...
    function test_compare_grd_lrd_fuzz(Init memory init, uint80 roundId) public override {
        ...
        if (init.yield > 0) {
            ...
            assertLe(
                ...
                "Asset: Answer is not lower than initial price without pool interactions despite negative yield (before expiry)"
            );
        }
