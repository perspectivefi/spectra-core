...
contract ContractRouterNGLiquidityTest is RouterNGBaseTest {
    ...
    uint256 private constant APPROXIMATION_TOLERANCE = 1e3;
    uint256 private constant APPROXIMATION_TOLERANCE_ONE_SIDED = 1e15; // 10 bps. dev: need to account for internl slippage
    ...
    function testAddLiquidityWithAssetFail() public {
        uint256 assets = 100e18;
        underlying.mint(MOCK_ADDR_1, assets);

        vm.startPrank(MOCK_ADDR_1);
        underlying.approve(address(router), assets);
        uint256 minExpectedLpAmount = routerUtil.previewNGAddLiquidityWithAsset(
            curvePoolAddr,
            assets
        );
    ...
    function testAddLiquidityWithIBTFail() public {
    ...
    function testAddAndRemoveLiquidityPrecise() public {
        ...
        uint256 ibtsStart = 80e15;
        uint256 ptsStart = 100e15;

        uint256 lpTokens = _testAddLiquidity([ibtsStart, ptsStart], MOCK_ADDR_1, MOCK_ADDR_2);
        uint256[2] memory coinsEnd = _testRemoveLiquidity(lpTokens, MOCK_ADDR_2, MOCK_ADDR_3);

        if (ibtsStart < 1e15) {
            assertApproxEqAbs(ibtsStart, coinsEnd[0] + _calcNoiseFee(ibtsStart), 50);
        } else {
            assertApproxEqRel(ibtsStart, coinsEnd[0] + _calcNoiseFee(ibtsStart), 1e8);
        }

        if (ptsStart < 1e15) {
            assertApproxEqAbs(ptsStart, coinsEnd[1] + _calcNoiseFee(ptsStart), 50);
        } else {
            assertApproxEqRel(ptsStart, coinsEnd[1] + _calcNoiseFee(ptsStart), 1e8);
        }
    }

    function testAddAndRemoveLiquidityOneCoin0Fuzz(uint256 amount) public {
        amount = bound(amount, 1e16, 1_000_000 * 1e18);
    ...
    function testAddAndRemoveLiquidityOneCoin1Fuzz(uint256 amount) public {
        amount = bound(amount, 1e15, 1_000_000 * 1e18);
    ...
    function testAddAndRemoveLiquidityWithAssetNoYieldFuzz(uint256 assets) public {
        uint256 assetsStart = bound(assets, 1e15, 1_000_000 * 1e18);
        ...
        if (assetsStart < 1e15) {
            assertApproxEqAbs(
                assetsStart,
                assetsEnd + _calcNoiseFee(assetsStart),
                20,
    ...
    function testAddAndRemoveLiquidityWithAssetVariableRateFuzz(
        uint256 assets,
        uint256 yieldFactor
    ) public {
        uint256 assetsStart = bound(assets, 1e15, 1_000_000 * 1e18);
    ...
    function testAddAndRemoveLiquidityWithIBTNoYieldFuzz(uint256 amount) public {
        amount = bound(amount, 1e15, 1_000_000 * 1e18);
        ...
        if (ibtsStart < 1e15) {
            assertApproxEqAbs(
                ibtsStart,
                ibtsEnd + _calcNoiseFee(ibtsStart),
                20,
    ...
    function testAddAndRemoveLiquidityWithIBTVariableRateFuzz(
        uint256 amount,
        uint256 yieldFactor
    ) public {
        amount = bound(amount, 1e15, 1_000_000 * 1e18);
    ...
    function testAddAndRemoveLiquidityMultipleUsersNoYieldFuzz(uint256 amount) public {
        amount = bound(amount, 1e15, 1_000_000 * 1e18);
    ...
    function testAddAndRemoveLiquidityMultipleUsersIncreasingYieldFuzz(
        uint256 amount,
        uint256 yieldFactor
    ) public {
        amount = bound(amount, 1e15, 1_000_000 * 1e18);
    ...
    function testYieldGenerationDoesNotChangeLPBalanceFuzz(
        uint256 amount,
        uint256 yieldFactor
    ) public {
        ...
        uint256 lpBalance = IERC20(curvePool).balanceOf(MOCK_ADDR_1);

        for (uint8 i = 0; i < 10; ++i) {
            _setYield(i * 0.25e18);
            assertTrue(
                IERC20(curvePool).balanceOf(MOCK_ADDR_1) == lpBalance,
    ...
    function _testAddLiquidity(
        ...
        data.lpBalReceiverBefore = IERC20(curvePool).balanceOf(receiver);
        ...
        uint256 minExpectedLpAmount = _getMinExpectedAmountWithTolerance(
            routerUtil.previewNGAddLiquidity(curvePoolAddr, amounts)
        );
        {
        ...
        data.lpBalReceiverAfter = IERC20(curvePool).balanceOf(receiver);
    ...
    function _testAddLiquidityWithAsset(
        ...
        underlying.approve(address(router), assets);
        uint256 minExpectedLpAmount = _getMinExpectedAmountWithTolerance(
            routerUtil.previewNGAddLiquidityWithAsset(curvePoolAddr, assets)
        );
        {
            (
    ...
    function _testAddLiquidityWithIBT(
        ...
        ibt.approve(address(router), ibts);
        uint256 minExpectedLpAmount = _getMinExpectedAmountWithTolerance(
            routerUtil.previewNGAddLiquidityWithIBT(curvePoolAddr, ibts)
        );
        {
            (
                bytes memory commands,
    ...
    function _testAddLiquidityOneCoin(
        ...
        IERC20 coin = IERC20(ICurveNGPool(curvePoolAddr).coins(i));
        ...
        uint256 minExpectedLpAmount = _getMinExpectedAmountWithToleranceOneSided(
            routerUtil.previewNGAddLiquidity(curvePoolAddr, amounts)
        );
        {
            (bytes memory commands, bytes[] memory inputs) = _buildRouterAddLiquidityExecution(
    ...
    function _testRemoveLiquidity(
        ...
        data.lpBalOwnerBefore = IERC20(curvePool).balanceOf(owner);
        ...
        vm.startPrank(owner);
        IERC20(curvePool).approve(address(router), lpTokens);
        uint256[2] memory minExpectedAmounts = routerUtil.previewNGRemoveLiquidity(
            curvePoolAddr,
            lpTokens
        );
        ...
        data.lpBalOwnerAfter = IERC20(curvePool).balanceOf(owner);
    ...
    function _testRemoveLiquidityOneCoin(
        ...
        vm.startPrank(owner);
        IERC20(curvePool).approve(address(router), lpTokens);
        uint256 minExpectedAmount = _getMinExpectedAmountWithToleranceOneSided(
            routerUtil.previewNGRemoveLiquidityOneCoin(curvePoolAddr, lpTokens, i)
        );
    ...
    function _buildRouterAddLiquidityExecution(
        ...
        address ibt = ICurveNGPool(curvePool).coins(0);
        address pt = ICurveNGPool(curvePool).coins(1);

        commands = abi.encodePacked(
            bytes1(uint8(Commands.TRANSFER_FROM)),
            bytes1(uint8(Commands.TRANSFER_FROM)),
            bytes1(uint8(Commands.CURVE_NG_ADD_LIQUIDITY))
        );
        inputs = new bytes[](3);
    ...
    function _buildRouterAddLiquidityWithAssetExecution(
        ...
        commands = abi.encodePacked(
            ...
            bytes1(uint8(Commands.CURVE_NG_SPLIT_IBT_LIQUIDITY)),
            bytes1(uint8(Commands.CURVE_NG_ADD_LIQUIDITY))
        );
        inputs = new bytes[](4);
    ...
    function _buildRouterRemoveLiquidityExecution(
        ...
        commands = abi.encodePacked(
            bytes1(uint8(Commands.TRANSFER_FROM)),
            bytes1(uint8(Commands.CURVE_NG_REMOVE_LIQUIDITY))
        );
        inputs = new bytes[](2);
        inputs[0] = abi.encode(curvePool, lpTokens);
    ...
    function _buildRouterRemoveLiquidityOneCoinExecution(
        ...
        commands = abi.encodePacked(
            bytes1(uint8(Commands.TRANSFER_FROM)),
            bytes1(uint8(Commands.CURVE_NG_REMOVE_LIQUIDITY_ONE_COIN))
        );
        inputs = new bytes[](2);
        inputs[0] = abi.encode(curvePool, lpTokens);
    ...
    function _getMinExpectedAmountWithTolerance(
        ...
        minMintAmount = (Constants.UNIT - APPROXIMATION_TOLERANCE).mulDiv(amount, Constants.UNIT);
    }

    /**
     * @dev Internal function that calculates the minimum expected amount with a tolerance for adding/removing liquidity with one coin.
     * Used when relying on Curve's previews (calc_token_amount() and calc_withdraw_one_coin()) that may not be 100% accurate.
     * @param amount The amount to deposit in the pool for a token.
     */
    function _getMinExpectedAmountWithToleranceOneSided(
        uint256 amount
    ) internal pure returns (uint256 minMintAmount) {
        minMintAmount = (Constants.UNIT - APPROXIMATION_TOLERANCE_ONE_SIDED).mulDiv(
            amount,
            Constants.UNIT
        );
    }
