...
contract DeployAllScript is Script {
    ...
    function run() public {
        ...
        if (forTest) {
            ...
            curvePoolParams.admin_fee = 5000000000;
            ...
            envVar = string.concat("CURVE_POOL_PARAM_ADMIN_FEE_", data.deploymentNetwork);
            if (bytes(vm.envString(envVar)).length == 0) {
                revert(string.concat(envVar, " is not set in .env file"));
            }
            curvePoolParams.admin_fee = vm.envUint(envVar);

            // get Curve Pool ma_half_time from .env
            envVar = string.concat("CURVE_POOL_PARAM_MA_HALF_TIME_", data.deploymentNetwork);
            if (bytes(vm.envString(envVar)).length == 0) {
                revert(string.concat(envVar, " is not set in .env file"));
            }
            curvePoolParams.ma_half_time = vm.envUint(envVar);
            if (curvePoolParams.ma_half_time == 0) {
                revert(string.concat(envVar, " cannot be 0"));
            }
    ...
    function deployForTest(
        ...
        curveAddressProvider = inputData._curveAddressProvider;
